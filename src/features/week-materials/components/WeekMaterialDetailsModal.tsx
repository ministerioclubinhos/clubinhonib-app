...
export default function WeekMaterialDetailsModal({
  ...
  return (
    <Dialog
      ...
      PaperProps={{
        sx: {
          width: '100%', m: '10px auto', p: 2, borderRadius: 3,
            ...
            <Paper elevation={2} sx={{ p: 3, borderRadius: 2, bgcolor: '#fff' }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <Typography variant="body1"><strong>Título:</strong> {material.title || 'Sem Título'}</Typography>
                </Grid>
                <Grid item xs={12} md={6}>
                  <Typography variant="body1"><strong>Subtítulo:</strong> {material.subtitle || 'Sem Subtítulo'}</Typography>
                </Grid>
                <Grid item xs={12}>
                ...
                <Grid item xs={12} md={6}>
                  <Typography variant="body1"><strong>Criado em:</strong> {formatDate(material.createdAt)}</Typography>
                </Grid>
                <Grid item xs={12} md={6}>
                  <Typography variant="body1"><strong>Atualizado em:</strong> {formatDate(material.updatedAt)}</Typography>
                </Grid>
              </Grid>
            </Paper>
                  ...
                  <Grid container spacing={3}>
                    {section.items.map((item) => (
                      <Grid item xs={12} md={6} key={item.id}>
                        <Box sx={{ p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#f9f9f9', height: '100%' }}>
                          <Typography variant="body1"><strong>Título:</strong> {item.title || 'Sem Título'}</Typography>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, gap: 1, flexWrap: 'wrap' }}>
                            ...
                            <IconButton size="small" onClick={() => handleCopyUrl(item.url)} title="Copiar URL" sx={{ color: '#1976d2' }}>
