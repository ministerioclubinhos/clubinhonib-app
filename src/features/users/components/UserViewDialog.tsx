...
export default function UserViewDialog({ open, user, onClose }: Props) {
  ...
  const waLink = useMemo(
    () =>
      user ? (buildWhatsappLink(user.name, loggedUser?.name, user.phone) ?? undefined) : undefined,
    ...
    >
      <Box
        sx={{
          ...
          pt: { xs: 2, sm: 3 },
          pb: { xs: 1.25, sm: 1.5 },
          background: 'linear-gradient(135deg, rgba(2,136,209,.08) 0%, rgba(76,175,80,.08) 100%)',
          borderBottom: 1,
          borderColor: 'divider',
        }}
      >
            ...
            <Stack direction="row" spacing={1.25} alignItems="center">
              ...
              <Box sx={{ minWidth: 0 }}>
                <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
                  <Typography variant="h6" fontWeight={800} noWrap={!isXs} title={user.name}>
                    {user.name || '—'}
                  </Typography>
                  <Chip
                    size="small"
