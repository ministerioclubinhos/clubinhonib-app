  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      const data: Page<TeacherProfile> = await apiListTeachers({
        ...(JSON.parse(filtersKey) as Pick<
          TeacherQuery,
          'searchString' | 'q' | 'active' | 'hasClub' | 'clubNumber'
        >),
        page: pageIndex + 1,
        limit: pageSize,
        sort: sortParam.sort,
...
export function useTeacherMutations(
  ...
  const setClub = useCallback(
    async (teacherId: string, clubId: string) => {
      setDialogLoading(true);
      setDialogError('');
      try {
        await apiAssignTeacherToClub(teacherId, clubId);
        await refreshOne(teacherId);
      } catch (err: any) {
        setDialogError(err?.response?.data?.message || err.message || 'Erro ao vincular Clubinho');
        throw err;
      } finally {
