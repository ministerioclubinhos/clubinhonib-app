// src/modules/children/ChildrenManager.tsx
...
import ChildDeleteDialog from "./components/ChildDeleteDialog";
...
export default function ChildrenManager() {
  ...
  const [sorting, setSorting] = useState<ChildSort>({ id: "updatedAt", desc: true });

  const { rows, total, loading, error, setError, fetchPage } =
    useChildren(pageIndex, pageSize, sorting, filters);
  ...
  const handleOpenView = (row: ChildResponseDto) => { setViewing(row); fetchChild(row.id); };

  // mutations
  const { dialogLoading, dialogError, setDialogError, createChild, updateChild, deleteChild } =
