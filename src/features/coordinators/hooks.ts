...
import type { ClubSimple, CoordinatorFilters, CoordinatorProfile } from './types';
import type { SortingState } from '@tanstack/react-table';

function mapSortingToServer(sorting: SortingState) {
  const first = sorting?.[0];
  const sortId =
    first?.id === 'user' ? 'name' : first?.id === 'createdAt' ? 'createdAt' : 'updatedAt';
...
function mapFiltersToServer(filters: CoordinatorFilters) {
  const active = filters.active === 'all' ? undefined : filters.active === 'active' ? true : false;
  const hasClubs =
    filters.hasClubs === 'all' ? undefined : filters.hasClubs === 'yes' ? true : false;
...
export function useCoordinatorProfiles(
  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err.message || 'Erro ao listar coordenadores');
    } finally {
      setLoading(false);
    }
  ...
  const assignClub = useCallback(
    async (coordinatorId: string, clubId: string): Promise<string> => {
      ...
      try {
        ...
        const msg = err?.response?.data?.message || err.message || 'Erro ao vincular clubinho';
        setDialogError(msg);
        throw err;
      } finally {
  ...
  const unassignClub = useCallback(
    async (coordinatorId: string, clubId: string): Promise<string> => {
      ...
      try {
        ...
        const msg = err?.response?.data?.message || err.message || 'Erro ao desvincular clubinho';
        setDialogError(msg);
        throw err;
      } finally {
        setDialogLoading(false);
