...
export async function apiListCoordinators(params: ListCoordinatorsParams) {
  const { data } = await api.get<PageDto<CoordinatorProfile>>('/coordinator-profiles', { params });
  return data;
}

export async function apiGetCoordinator(coordinatorId: string) {
  const { data } = await api.get<CoordinatorProfile>(`/coordinator-profiles/${coordinatorId}`);
  return data;
}

export async function apiAssignClub(coordinatorId: string, clubId: string): Promise<ApiMessage> {
...
export async function apiUnassignClub(coordinatorId: string, clubId: string): Promise<ApiMessage> {
  const { data } = await api.patch<ApiMessage>(
