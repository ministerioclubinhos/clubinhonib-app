...
export default function CommentDetailsModal({ comment, open, onClose }: Props) {
  return (
    <Dialog
      ...
      maxWidth="md"
      PaperProps={{
        sx: { width: '100%', m: '10px auto', p: '5px', borderRadius: 3, bgcolor: '#fafafa' },
      }}
    >
      <DialogTitle
        textAlign="center"
        fontWeight="bold"
        sx={{ fontSize: '1.5rem', color: '#333', p: '5px' }}
      >
            ...
            <Paper elevation={1} sx={{ p: '5px', borderRadius: 2, bgcolor: '#fff' }}>
              ...
              >
                ...
                >
                  ...
                  <Grid item xs={12} md={6}>
                    <Typography variant="body1">
                      <strong>Criado em:</strong> {fmtDate(comment.createdAt)}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Typography variant="body1">
                      <strong>Atualizado em:</strong> {fmtDate(comment.updatedAt)}
                    </Typography>
                  </Grid>
                  {comment.id && (
                    <Grid item xs={12} md={6}>
